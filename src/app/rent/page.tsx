...
export default function RentPage() {
  ...
  async function fetchCars() {
    try {
      setLoading(true);
      setError(null);
      const res = await fetch("/api/public/cars?agency=demo");
      const json = await res.json();
      if (json.success) {
        setCars(json.data ?? []);
      } else {
        setError(json.error ?? "Failed to load cars");
      }
    } catch {
  ...
  return (
    <>
      ...
      <section className="w-full max-w-container-max-width px-margin-mobile md:px-margin-desktop pb-12">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="font-montserrat text-headline-lg-mobile text-on-surface mb-2">No cars available</p>
            <p className="font-manrope text-body-md text-on-surface-variant">Check back later for new listings</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {cars.map((car) => (
