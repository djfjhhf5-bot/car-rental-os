...
export async function POST(request: NextRequest) {
  try {
    ...
    const { prisma } = await import("@/lib/prisma");
    const configs = await prisma.wassenderConfig.findMany({
    ...
    if (!conversation) {
      conversation = await prisma.conversation.create({
        data: { phoneNumber: digits, agencyId, leadId: matchedLead?.id || null, messages: [], lastMessageId: null, messageCount: 0, pendingReply: null, retryCount: 0 },
      });
    } else if (conversation.lastMessageId === messageId) {
    ...
    const trySend = async (replyText: string): Promise<boolean> => {
      try {
    ...
    const storeReply = async (replyText: string) => {
      await prisma.conversation.update({
        where: { id: conversation.id },
        data: { pendingReply: replyText, retryCount: { increment: 1 }, messages: recentMessages, messageCount: recentMessages.length, lastMessageId: messageId, updatedAt: new Date() },
      });
    };

    if (pendingReply) {
      if (await trySend(pendingReply)) {
        replied = true;
        ...
        await prisma.conversation.update({
          where: { id: conversation.id },
          data: { messages: recentMessages, messageCount: recentMessages.length, lastMessageId: messageId, updatedAt: new Date() },
        });
        return Response.json({ success: true, data: { received: true, replied: false, queued: true } });
      }
    }
    ...
    if (llmResult.success && llmResult.data) {
      ...
      const leadContext = matchedLead ? { name: matchedLead.name, phase: matchedLead.phase, source: matchedLead.source || undefined, notes: matchedLead.notes || undefined } : null;
    ...
    if (!replyText) {
      replyText = `Hi! 👋\n\nBrowse our cars: ${fleetCtx.carsPageUrl}\n- ${fleetCtx.agencyName}`;
    }

    if (await trySend(replyText)) {
      replied = true;
      ...
      await storeReply(replyText);
    }

    return Response.json({ success: true, data: { received: true, replied, queued: !replied } });
  } catch (error) {
